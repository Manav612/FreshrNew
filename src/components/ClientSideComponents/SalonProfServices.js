...
import { BASE_API_URL } from '../../Services';
  ...
  const SalonProfServices = ({ route }) => {
    const theme = useSelector(state => state.ThemeReducer);
    const COLOR = theme == 1 ? COLOR_DARK : COLOR_LIGHT;
    const dispatch = useDispatch();
    const { FacilityDetail,ProfDetail  } = route.params;
   console.log("==========  profff id  ===========",ProfDetail._id);
   console.log("==========  FacilityDetail   ===========",FacilityDetail.location);
   const facilityAddress = FacilityDetail.location
    const ProfID = ProfDetail._id;
    ...
    const fetchServicesData = async () => {
      try {
        ...
        const res = await axios.get(
          `${BASE_API_URL}/professionals/professional/${ProfID}`,
          config,
        );
        console.log(
          '=======   fetchhh services  == ========',
  ...
  export default SalonProfServices;
